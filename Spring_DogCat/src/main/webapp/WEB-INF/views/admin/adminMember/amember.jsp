     ...
     <tbody>
 		...
 		<c:forEach items = "${mList}" var = "aMemberDTO" begin="${beginNum }" end="${endNum }">  <!-- items 쌓여있는곳 var -> for(int i 이부 -->
			<tr>
				...
				<td align="center"><a href ="amember_del?mNo=${aMemberDTO.mNo}">${aMemberDTO.mState}</a></td>
			...
			<tr>
				...
				<a href="amemberPaging?page=${status.count}">${status.count}</a>
			</c:forEach></td>
		...
		<div align ="center">
		<!-- 검색 SelectBox -->
		<form action="amember" method="post">
		검색 선택 :
			<select name="query" >
